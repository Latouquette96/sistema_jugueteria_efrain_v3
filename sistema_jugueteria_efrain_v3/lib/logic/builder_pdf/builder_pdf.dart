...
class BuilderPDF {
  ...
  static Future<ResponseSystem<File>> buildPDF({required WidgetRef ref, required List<Product> list}) async {
    ...
    ResponseSystem<File> responseSystem;
    ...
    try{
      ...
      for (int i = 0; i < listPDF.length; i++) {
        ...
        if (await fileImg.exists()) {
          ...
          try {
            image = pw.Image(
                await networkImage(listPDF[i].getLinkImages()[0].getLink()));
          } catch (d) {
            image = imageTemp;
          }
        }

        var styleStock = pw.TextStyle(
            font: fontRegular,
            fontSize: 11,
            color: _getColorStock(e.getStock()));

        List<pw.Row> listRow = [];
        listRow.add(pw.Row(children: [
          pw.Text(e.getTitle().toUpperCase(), style: styleTitle)
        ]));
        listRow.add(pw.Row(children: [
        ...
        listRow.add(pw.Row(children: [
          pw.Text("Precio público: ", style: styleLabel),
          pw.Text("\$${e.getPricePublic().toStringAsFixed(2)}",
              style: styleInfo)
        ]));
        listRow.add(pw.Row(children: [
          pw.Text("Stock: ", style: styleLabel),
          pw.Text(_getLabelStock(e.getStock()), style: styleStock)
        ]));
        listRow.addAll(e.getSizes().map(
          (e) {
            return pw.Row(children: [pw.Text(e, style: styleLabel)]);
          },
        ));

        listProductPDF.add(pw.Partition(
            width: PdfPageFormat.a4.width - PdfPageFormat.a4.marginRight,
            child: pw.Container(
                padding: const pw.EdgeInsets.all(5),
                color: i % 2 == 0
                    ? PdfColor.fromRYB(0, 0.25, 0)
                    : PdfColor.fromRYB(0, 0.35, 0),
                width: PdfPageFormat.a4.width,
                height: 135,
                child: pw.Row(mainAxisSize: pw.MainAxisSize.max, children: [
                  pw.Container(
                      width: 125,
                      height: 125,
      ...
      pw.Page page = pw.MultiPage(
          ...
          header: (pw.Context context) {
            return pw.Container(
                ...
                child: pw.Row(
                    ...
                    children: [
                      pw.Text("Jugueteria Efrain - Catálogo de Productos*",
                          style: styleHeader),
                      pw.Text("Fecha: ${DatetimeCustom.getDatetimeStringNow()}",
                          style: styleHeader)
                    ]));
          },
          footer: (pw.Context context) {
            return pw.Container(
                height: 60,
                child: pw.Text(
                    "*El precio de los productos es válido durante todo el día. "
      ...
      responseSystem = ResponseSystem<File>.manual(status: 500, value: null, title: "Error", message: "Error: Ocurrió un problema. El archivo PDF no pudo ser generado.");
    }
      ...
      List<pw.Partition> listProductPDF = listPDF.map((e) {
        ...
        return pw.Partition(
            ...
            child: pw.Container(
                ...
                color:
                    (current % 2 == 0) ? PdfColors.amber50 : PdfColors.amber100,
                width: PdfPageFormat.a4.width,
                height: 25,
                child: pw.Row(
                    mainAxisSize: pw.MainAxisSize.max,
                    ...
                    children: [
                      ...
                      pw.Container(
                          width: 225,
                          child: pw.Text(
                              e.getValue1().getTitle().toUpperCase().substring(
                                  0,
                                  (e.getValue1().getTitle().length < 30)
                                      ? e.getValue1().getTitle().length
                                      : 30),
                              style: styleLabel)),
                      pw.Container(
                          width: 50,
                          child: pw.Text("(${e.getValue2().toString()} un)",
                              style: styleLabel)),
                      pw.Container(
                          width: 125,
                          child: pw.Text(
                              "\$${e.getValue1().getPricePublic().toStringAsFixed(2)} (\$${e.getValue3()!.toStringAsFixed(2)} un)",
                              style: styleLabel)),
                      pw.Container(
                          width: 125,
                          child: pw.Text(
                              "\$${(e.getValue1().getPricePublic() * e.getValue2()!).toStringAsFixed(2)} (\$${(e.getValue3()! * e.getValue2()!).toStringAsFixed(2)})",
                              style: styleLabel)),
                    ])));
      }).toList();
          ...
          header: (pw.Context context) {
            return pw.Container(
                ...
                child: pw.Row(
                    ...
                    mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                    children: [
                      pw.Text("Jugueteria Efrain - Detalle de la venta",
                          style: styleHeader),
            ...
            return pw.Column(children: [
              pw.Container(
                  ...
                  child: pw.Row(
                      ...
                      mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                      children: [
                        pw.Text("CANTIDAD DE PRODUCTOS: $cantProducts",
                            style: styleLabel),
                      ])),
              pw.Container(
                  padding: const pw.EdgeInsets.all(5),
                  ...
                  child: pw.Row(
                      ...
                      children: [
                        pw.Text("SUBTOTAL: \$${subtotal.toStringAsFixed(2)}",
                            style: styleTitle),
                        pw.Text("DESCUENTO: \$${descuento.toStringAsFixed(2)}",
                            style: styleTitle),
                        pw.Text("RECARGO: \$${recargo.toStringAsFixed(2)}",
                            style: styleTitle),
                        pw.Text(
                            "TOTAL: \$${(subtotal + recargo + descuento).toStringAsFixed(2)}",
                            style: styleTitle),
                      ]))
            ]);
          });
